...
def test_rolling_live_event_targets_stay_on_today_before_peak_cutoff():
    ...
    assert targets == [date(2026, 6, 10)]
    assert events_by_target[date(2026, 6, 10)].event_ticker == "KXHIGHTSFO-26JUN10"


def test_rolling_live_event_targets_switch_to_next_day_after_peak_cutoff():
